...
plot(largest_clique, layout=layout.kamada.kawai(largest_clique))
...
### 군집 갯수 정하기
g25 <- cutree(hc, k = 2:5) # 군집을 2~5개로 나눠보기
g25
...
for(cluster in clusters) {
  c <- do.call(cluster, list(simplify(g_informal_s))) 
  plot(c, 
        g_informal, vertex.shape="none",  
        edge.arrow.mode=0, 
        layout=l, 
        main=paste0(cluster,"(",max(c$membership), ")")) }
dev.off()
